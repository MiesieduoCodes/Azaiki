      ...
      {filteredArtists.map((artist) => (
        <motion.div
          ...
          <div className="p-6">
            ...
            <p className="mt-2 font-[Noto Sans] ">
              {artist.occupation}
            </p>
          </div>
        </motion.div>
        ...
        <div className="px-4 sm:px-6 pt-24 py-8 mt-10">
          ...
          {featuredArtist && (
            <div className="max-w-4xl mx-auto bg-yellow-400 p-6 rounded-xl shadow-md text-center">
              ...
              <img src={featuredArtist.profileImage} alt={featuredArtist.name} className="mx-auto mt-4 w-32 h-32 rounded-full object-cover"/>
          ...
          {artists.length > visibleArtists && (
            <div className="mt-6 text-center">
              <button onClick={() => setVisibleArtists(visibleArtists + 6)} className="px-4 py-2 bg-yellow-400 text-black rounded-md hover:bg-yellow-500 transition duration-200 font-[Roboto Condensed]">
                Load More
              </button>
            </div>
          )}
